...
|分辨率|基础视觉嵌入|3组DeepStack|完整状态|
|---|---:|---:|---:|
|低|0.625MB|1.875MB|2.500MB|
|中|2.3438MB|7.0312MB|9.375MB|
|高|9.6875MB|29.0625MB|38.750MB|

补充验证中观察到的实际完整状态大小为low页面7.875MB、medium页面30.375MB。该数值不能直接与上表比较，除非页面经过处理器后具有相同的`image_grid_thw`和视觉token数。后续结果必须同时记录：
